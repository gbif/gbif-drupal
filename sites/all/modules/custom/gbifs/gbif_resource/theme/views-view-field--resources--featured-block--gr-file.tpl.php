 ...
 */
if (!empty($row->field_gr_file)) {
	$lang_code = $row->_field_data['nid']['entity']->language;
	$link_title = t('Download') . ' ' . $row->_field_data['nid']['entity']->gr_file[$lang_code][0]['description'];
	$link_path = 'system/files_force' . substr($row->_field_data['nid']['entity']->gr_file[$lang_code][0]['uri'],  8); // Strip off file schema.
	$img = theme_image(
		array(
	...
	$link = l($img, $link_path, array(
		'html' => TRUE,
		'attributes' => array(
			'type' => $row->_field_data['nid']['entity']->gr_file[$lang_code][0]['filemime'],
		),
		'query' => array(
			'download' => 1,
		),
