 ...
 */
if (!empty($row->_field_data['gr_url']['entity']->gr_url)) {
	$lang_code = $row->_entity_properties['entity object']->language;
	$link_title = $row->_field_data['gr_url']['entity']->title;
	$link_path = $row->_field_data['gr_url']['entity']->gr_url[$lang_code][0]['value'];
	$img = theme_image(
		array(
