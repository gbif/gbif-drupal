...
/**
 * @file
 * Contains \Drupal\gbif_restful\Plugin\formatter\FormatterGbifJson.
 */

namespace Drupal\gbif_restful\Plugin\formatter;
 ...
 *
 * @package Drupal\gbif_restful\Plugin\formatter
 *
 * @Formatter(
 *   id = "json_gbif",
 *   label = "GBIF JSON",
 *   description = "Output in JSON format that is closer to other GBIF APIs."
 * )
 */
class FormatterGbifJson extends Formatter implements FormatterInterface {
  ...
  public function prepare(array $data) {
    ...
    $output = array('results' => $this->limitFields($extracted));
  ...
  protected function extractFieldValues($data, array $parents = array(), array &$parent_hashes = array()) {
    ...
    foreach (['field_uni_images', 'field_programme_ef_image', 'field_pj_image'] as $image_field) {
      if (isset($output[$image_field]) && $output[$image_field] == null) {
        unset($output[$image_field]);
      }
      else {
        if (isset($output[$image_field])) $output['images'] = $output[$image_field];
        unset($output[$image_field]);
      }
    }
