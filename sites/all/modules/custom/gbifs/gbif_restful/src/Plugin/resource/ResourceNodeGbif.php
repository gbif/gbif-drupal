...
class ResourceNodeGbif extends ResourceNode implements ResourceNodeGbifInterface {
  ...
  protected function publicFields() {
    ...
    if (count($bundle) == 1) {
      $bundle = $bundle[0];
      if (field_info_instance($this->getEntityType(), 'field_subtitle', $bundle) != NULL) {
        $public_fields['subtitle'] = array(
          'property' => 'field_subtitle',
        );
      }
      if (field_info_instance($this->getEntityType(), 'field_title_for_display', $bundle) != NULL) {
  ...
  public function getEntitySelf(DataInterpreterInterface $interpreter) {
    return parent::getEntitySelf($interpreter); // TODO: Change the autogenerated stub
  }
  ...
  public function relatedResourcesProcess($value) {
    ...
    return array(
      'url' => (strpos($value['url'], 'http') == 0) ? $value['url'] : $base_url . $value['url'],
      'title' => $value['title'],
  ...
  private static function getTranslatedNodes($node) {
    ...
    switch ($node->type) {
      case 'news':
        $reference_field = 'field_translation_source';
        break;
      case 'generic':
        $reference_field = 'field_translation_source_generic';
        break;
    }
